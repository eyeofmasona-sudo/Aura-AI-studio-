  ...
  const generateCharactersFromIdea = async () => {
    ...
    try {
      ...
      const result = await AiStore.getInstance().requestExecution({
        ...
        systemInstruction: "You are an expert movie director. Given project metadata, respond ONLY with a valid JSON array of 3-4 distinct characters. Do not output any markdown code blocks, conversational text, or explanations. Direct raw JSON array matching this exact schema: Array<{ name: string, role: string, age: string, description: string, appearance: string, outfit: string, personality: string, goal: string, conflict: string }>. CRITICAL DIRECTIVE 1: If the movie context contains any specific Country name (e.g. Армения/Armenia, Япония/Japan, etc.), you MUST strictly tailor each character's appearance description ('appearance') and outfit styling ('outfit') to accurately match the national, ethnic, and cultural look of that country or region. CRITICAL DIRECTIVE 2: If the provided movie context (idea, synopsis, logline) contains explicit character names and their descriptions, you MUST use those exact names and base the generated characters entirely on them. Do not invent new main characters if they are already named in the context. Your output must be syntactically perfect JSON."
      });
