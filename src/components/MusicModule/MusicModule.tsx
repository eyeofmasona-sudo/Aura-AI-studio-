...
${refCount > 0 ? `Ориентироваться на загруженное референс-аудио (${refCount} шт.)` : ""}`;
    ...
    alert("Music Prompt успешно собран на основе параметров!");
  };

  const improveMusicPrompt = () => {
    if (!state.musicPrompt) {
      alert("Сначала соберите или напишите базовый промпт!");
      return;
    }
    setState(prev => ({ ...prev, isGenerating: true }));
    setTimeout(() => {
      setState(prev => {
        const enriched = `${prev.musicPrompt}\n\n[ИИ Уточнение]: Добавить плотный субпадовый бас, частоту реверберации Hall Reverb на отметке 65%, стереорасширение на струнной группе, плавную компрессию саундтрека по стандарту LUFS -14 для профессионального стримингового релиза. Сформировать плотный саббитный бит в кульминации.`;
        const updated = {
          ...prev,
          musicPrompt: enriched,
          isGenerating: false
        };
        saveGameState(updated);
        return updated;
  ...
  // Suggest helpers: AI parameter actions
  const suggestMusicGenre = () => {
    const recommended = "synthwave";
    updateState({ selectedMusicGenre: recommended });
    alert("ИИ порекомендовал жанр: " + recommended.toUpperCase() + " (на основе лора истории)");
  };

  const suggestMusicMood = () => {
    const recommended = "напряжённое";
    updateState({ selectedMusicMood: recommended });
    alert("ИИ порекомендовал настроение: " + recommended.toUpperCase() + " (соответствует пику драмы)");
  };

  const suggestInstruments = () => {
    const instruments = ["strings", "synth", "pads", "bass"];
    updateState({ selectedInstruments: instruments });
    alert("ИИ активировал инструменты: Strings, Synth, Pads, Bass");
  };

  const suggestBpm = () => {
    updateState({ selectedBpm: "medium", customBpm: "110" });
    alert("ИИ установил идеальный темп сцены: Medium (110 BPM)");
  };

  const generateSceneCue = () => {
    if (state.importedScenes.length === 0) {
      alert("Пожалуйста, сначала импортируйте сцены!");
      return;
    }
    // Update first cue notes and values with AI wizard suggestion
    setState(prev => {
      const updated = prev.sceneCues.map((c, idx) => {
        if (idx === 0) {
          return {
            ...c,
            startMood: "таинственное",
            endMood: "мрачное",
            intensity: 8,
            notes: "[Сгенерировано ИИ] Начать с одиночного аналогового арпеджиатора, в середине сцены ввести низкие виолончели."
          };
        }
        return c;
      });
      const res = { ...prev, sceneCues: updated };
      saveGameState(res);
      return res;
    });
    alert("Сгенерировано детальное руководство перехода для Сцены 1!");
  };

  const generateSfxList = () => {
    const generatedSfx: SFXItem[] = [
      { id: "sfx-1", name: "Шлепки тяжелых капель", sceneName: "Сцена 1. Пробуждение в переулке", timecode: "00:01", description: "Низкочастотный глухой гул от падающей воды нарушает тишину" },
      { id: "sfx-2", name: "Глухой скрежет двери бара", sceneName: "Сцена 2. Встреча в Синт-Баре", timecode: "00:45", description: "Металлический скрип старых гидравлических петель" },
      { id: "sfx-3", name: "Свист двигателя флаера", sceneName: "Сцена 3. Погоня на Флаерах", timecode: "01:20", description: "Прогрессивное ионное шипение пролетающего корабля" }
    ];
    updateState({ sfxList: generatedSfx });
    alert("Библиотека синхронизированных звуковых эффектов (SFX) создана!");
  };

  const generateAudioDirection = () => {
    const sug = {
      id: `dir-${Date.now()}`,
      title: "Глобальная аудио-дирекция проекта",
      text: "Общий саундтрек строится на полярных тонах: холодные цифровые синтезаторы контрастируют с теплым аналоговым фортепиано. Локации окрашиваются ревербератором: переулки имеют широкое эхо, а синт-бары — короткую отражающую компрессию помещения.",
      type: "expert"
    };
    setState(prev => {
      const u = {
  ...
  const saveMusicModule = () => {
    alert("Музыкальное направление проекта успешно утверждено и сохранено в глобальную базу спецификации!");
    onApprove();
  };

  const sendToAudioEditor = () => {
    alert(`Передано треков референсов: ${state.uploadedAudioReferences.length} и сгенерированных cues в Студийный Аудиоредактор!`);
  };

  const sendToVideoEditor = () => {
    alert(`Всего ${state.sceneCues.length} временных отметок cues синхронизировано со Списком Сцен в Видеоредакторе!`);
  };
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              ...
              <button 
                onClick={() => {
                  ...
                  alert("В промпт добавлена оркестровая глубина!");
                }}
              ...
              <button 
                onClick={() => {
                  ...
                  updateState({ musicPrompt: enriched });
                  alert("Саундтрек адаптирован под дождливую уличную сцену!");
                }}
                className="p-2.5 bg-black/40 hover:bg-slate-900 border border-slate-800 hover:border-slate-700 rounded-lg text-xs font-bold text-slate-300 text-left flex items-center gap-2"
              >
            ...
            >
              ...
              <button
                onClick={() => {
                  ...
                  alert("Идея добавлена в промптер!");
                }}
